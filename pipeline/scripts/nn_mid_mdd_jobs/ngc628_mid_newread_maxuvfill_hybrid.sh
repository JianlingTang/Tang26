...
#PBS -l jobfs=10GB
#PBS -l wd
#PBS -N nn_ngc628_newread
#PBS -j oe
#PBS -m bea
#PBS -M janet.tang@anu.edu.au
#PBS -o /scratch/jh2/jt4478/output/nn_ngc628_mid_newread_maxuvfill_hybrid.o
#PBS -e /scratch/jh2/jt4478/output/nn_ngc628_mid_newread_maxuvfill_hybrid.e
...
RESTART="${RESTART:-0}"
OUTNAME="${OUTNAME:-ngc628_mid_newread_maxuvfill_tangnn_${POBS_MODE}.h5}"
...
{
  echo "RUN=new reader no photerr>0.3 cut + max-mag missing UV/U fill"
  echo "SCRIPT=${BUNDLE_DIR}/analyze_mid_mdd_new_read.py"
  echo "GALAXY_NAMES=ngc628-c ngc628-e"
