...
if [[ -n "${GALAXY_NAMES:-}" ]]; then
  ...
  read -r -a GALAXY_ARGS <<< "${GALAXY_NAMES_NORMALIZED}"
  GALAXY_LABEL="${GALAXY_LABEL:-${GALAXY_NAMES_NORMALIZED// /_}}"
elif [[ -n "${GALAXY:-}" ]]; then
  GALAXY_NORMALIZED="${GALAXY//,/ }"
  read -r -a GALAXY_ARGS <<< "${GALAXY_NORMALIZED}"
  GALAXY_LABEL="${GALAXY_LABEL:-${GALAXY_NORMALIZED// /_}}"
else
