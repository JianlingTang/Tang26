...
#PBS -l jobfs=10GB
#PBS -l wd
#PBS -N nn_ngc3344_mdd_newread
#PBS -j oe
#PBS -m bea
#PBS -M janet.tang@anu.edu.au
#PBS -o /scratch/jh2/jt4478/output/nn_ngc3344_mdd_newread_maxuvfill_priorcache_hybrid.o
#PBS -e /scratch/jh2/jt4478/output/nn_ngc3344_mdd_newread_maxuvfill_priorcache_hybrid.e
...
RESTART="${RESTART:-0}"
OUTNAME="${OUTNAME:-ngc3344_mdd_newread_maxuvfill_priorcache_tangnn_${POBS_MODE}.h5}"
...
LOG_FILE="${LOG_DIR}/${SAFE_OUTNAME}_ngc3344_mdd.log"
...
{
  echo "RUN=NGC3344 MDD new reader no photerr>0.3 cut + max-mag missing UV/U fill + prior-cache weights"
  echo "SCRIPT=${BUNDLE_DIR}/analyze_mid_mdd_new_read.py"
  echo "GALAXY_NAMES=ngc3344"
