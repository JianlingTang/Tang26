...
PARAMS_OBSERVED_BOX = np.array(
    [-1.12651935, 3.45569034, -0.30280910, 6.85636875, -0.42589259,
     -0.82499666, -1.31327108, -2.35789549, -1.04556497, -0.66185703],
    dtype=float,
)
PARAMS_HYBRID_MV6 = np.array(
    [-1.01624893, 3.29266678, -0.79281333, 9.89743440, -0.43033498,
     -0.98114892, -1.04674760, -1.99575287, -1.15807281, -0.43376576],
    dtype=float,
)
PARAMS_NN = np.array(
    [-0.10679347, 3.25002287, -0.62830372, 6.76424854, 0.01919158,
     -0.42239928, -1.82563080, -1.61655792, -1.69788263, -1.24958697],
    dtype=float,
)
PARAMS_OBSERVED_BOX_M2 = PARAMS_OBSERVED_BOX.copy()
PARAMS_OBSERVED_BOX_M2[0] = -2.0
PARAMS_HYBRID_MV6_M2 = PARAMS_HYBRID_MV6.copy()
PARAMS_HYBRID_MV6_M2[0] = -2.0
...
OUT_PNG = OUTDIR / "ngc3344_observed_vs_slug_predicted_magnitudes_mid_fixed_alpha_m2_compare.png"
OUT_PDF = OUTDIR / "ngc3344_observed_vs_slug_predicted_magnitudes_mid_fixed_alpha_m2_compare.pdf"
OUT_NPZ = OUTDIR / "ngc3344_observed_vs_slug_predicted_magnitudes_mid_fixed_alpha_m2_compare_hist.npz"
OUT_4PANEL_PNG = OUTDIR / "ngc3344_observed_vs_slug_predicted_magnitudes_4scenario_mid_fixed_alpha_m2_compare.png"
OUT_4PANEL_PDF = OUTDIR / "ngc3344_observed_vs_slug_predicted_magnitudes_4scenario_mid_fixed_alpha_m2_compare.pdf"
OUT_4PANEL_NPZ = OUTDIR / "ngc3344_observed_vs_slug_predicted_magnitudes_4scenario_mid_fixed_alpha_m2_compare_hist.npz"
...
SCENARIOS = [
    ("observed-box best", PARAMS_OBSERVED_BOX.copy(), "observed-box"),
    (r"observed-box $\alpha_M=-2$", PARAMS_OBSERVED_BOX_M2.copy(), "observed-box"),
    ("hybrid mv6 best", PARAMS_HYBRID_MV6.copy(), "hybrid"),
    (r"hybrid mv6 $\alpha_M=-2$", PARAMS_HYBRID_MV6_M2.copy(), "hybrid"),
]
...
def main() -> None:
    ...
    clean_legus(
        ...
        nn_scaler_path=None,
        nn_model_path=None,
        comp_threshold=CLEAN_COMP_THRESHOLD,
        ...
        pobs_specs.append(
            {
                ...
                "lib_indices_nn": [filter_names.index(f) for f in subset_filters],
                "nn_full_order": [str(f) for f in cat["filters"]],
                "nn_missing_band_fills": nn_uv_u_fills,
                "bounds_lo": bounds_lo,
                "bounds_hi": bounds_hi,
                "calc": calc,
    ...
    hist_obs_err: dict[str, np.ndarray] = {}
    ...
    for filt in allfilters:
        ...
        norm = max(float(np.sum(obs_counts)), 1.0)
        h_obs = obs_counts / (norm * bin_widths)
        h_obs_err = np.sqrt(obs_counts) / (norm * bin_widths)
        hist_obs[filt] = h_obs
        hist_obs_err[filt] = h_obs_err
    ...
    chunk_rows = 200_000
    n_seen = 0
    pobs_kept = [0 for _ in pobs_specs]
    pobs_positive_sum = [0.0 for _ in pobs_specs]
    pobs_positive_n = [0 for _ in pobs_specs]
    print(
    ...
    with fits.open(PHOT_FITS, memmap=True) as phot_hdul, fits.open(PROP_FITS, memmap=True) as prop_hdul:
        ...
        for start in range(0, n_total, chunk_rows):
            ...
            chunk_comps = []
            for k, spec in enumerate(pobs_specs):
                ...
                comp_by_mode = {
                    "nn": np.asarray(out["comp_nn"], dtype=float),
                    "observed-box": np.asarray(out["comp_nn"], dtype=float)
                    * np.asarray(out["inside_5d"], dtype=float),
                    "hybrid": np.asarray(out["comp_hybrid"], dtype=float),
                    "flat": 1.0,
                }
                chunk_comps.append(comp_by_mode)
                comp_for_stats = comp_by_mode["observed-box"]
                positive = comp_for_stats > 0.0
                pobs_kept[k] += int(np.sum(comp_for_stats >= 0.01))
                pobs_positive_sum[k] += float(np.sum(comp_for_stats[positive]))
                pobs_positive_n[k] += int(np.sum(positive))
            for si, (_label, _params, pobs_mode) in enumerate(SCENARIOS):
                for k, spec in enumerate(pobs_specs):
                    subset_filters = spec["subset_filters"]
                    comp = chunk_comps[k][pobs_mode]
    ...
    np.savez(
        ...
        params=PARAMS_OBSERVED_BOX,
        **{f"obs_{short_filter(f)}": hist_obs[f] for f in allfilters},
        **{f"obs_err_{short_filter(f)}": hist_obs_err[f] for f in allfilters},
