...
OUT_PNG = OUTDIR / "ngc3344_observed_vs_slug_predicted_magnitudes_observed_box_only_current_params.png"
OUT_PDF = OUTDIR / "ngc3344_observed_vs_slug_predicted_magnitudes_observed_box_only_current_params.pdf"
OUT_NPZ = OUTDIR / "ngc3344_observed_vs_slug_predicted_magnitudes_observed_box_only_current_params_hist.npz"
OUT_4PANEL_PNG = OUTDIR / "ngc3344_observed_vs_slug_predicted_magnitudes_4scenario_observed_box_only_current_params.png"
OUT_4PANEL_PDF = OUTDIR / "ngc3344_observed_vs_slug_predicted_magnitudes_4scenario_observed_box_only_current_params.pdf"
OUT_4PANEL_NPZ = OUTDIR / "ngc3344_observed_vs_slug_predicted_magnitudes_4scenario_observed_box_only_current_params_hist.npz"
...
SCENARIOS = [
    ("current params + NN pobs x box", PARAMS.copy(), "nn"),
    ("current params + flat pobs", PARAMS.copy(), "flat"),
    (r"$\alpha_M=-2$ + NN pobs x box", PARAMS_ALPHA_M2.copy(), "nn"),
...
def main() -> None:
    ...
    with fits.open(PHOT_FITS, memmap=True) as phot_hdul, fits.open(PROP_FITS, memmap=True) as prop_hdul:
        ...
        for start in range(0, n_total, chunk_rows):
            ...
            for k, spec in enumerate(pobs_specs):
                idx_rows = np.arange(stop - start, dtype=int)
                comp_nn = batched_joint_nn_libcomp(
                    phot_neb_ex,
                    idx_rows,
                    spec["lib_indices_nn"],
                    float(cat["dmod"]),
                    ...
                    model_path,
                    65536,
                    missing_band_fills=spec["nn_missing_band_fills"],
                )
                ...
                comp = comp_nn * inside_box.astype(float)
    ...
    fig.suptitle(
        "NGC3344 observed vs SLUG predicted magnitudes\nNN no hybrid criteria; observed box only",
        fontsize=12,
        y=0.997,
    )
